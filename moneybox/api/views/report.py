...
class ReportViewSet(viewsets.ViewSet):
    ...
    def get_total_incomes(profile, start_date=None, end_date=None):
        total_incomes_per = Income.objects.filter(
            created_by=profile,
            created_at__range=ReportViewSet.get_start_end_dates(start_date,
                                                                end_date)
        ).aggregate(total_incomes=Sum('amount')).get('total_incomes') or 0

        total_incomes = Income.objects.filter(created_by=profile).aggregate(
    ...
    def get_total_expenses(profile, start_date=None, end_date=None):
        total_expenses_per = Expense.objects.filter(
            created_by=profile,
            created_at__range=ReportViewSet.get_start_end_dates(start_date,
                                                                end_date)
        ).aggregate(total_expenses=Sum('amount')).get('total_expenses') or 0

        total_expenses = Expense.objects.filter(created_by=profile).aggregate(
    ...
    def get_category_incomes(profile, start_date=None, end_date=None):
        ...
        return [
            {**x, 'created_at': ReportViewSet.convert_date(x['created_at'])}
            for x in category_incomes
        ]
    ...
    def get_queryset(profile, start_date=None, end_date=None):
        ...
        results = {
            ...
            'category_incomes': ReportViewSet.get_category_incomes(
                profile,
                start_date,
                end_date),
            'category_expenses': ReportViewSet.get_category_expenses(
                profile,
                start_date,
                end_date),
        }
